...
| `wiki-narrate`             | Developed, cited briefing on a transversal topic (long exposé)           | `/wiki-narrate`                                  |
| `wiki-import`              | Import a graph.json or markdown bundle (inverse of export)               | `/wiki-import`                                   |
| `vault-skill-factory`      | Turn a mature page cluster into a portable expert skill                  | `/vault-skill-factory`                           |
...
```
[[wiki/javascript/stale-closure]]
[[wiki/outils/react]]
```

The `wiki/` prefix makes it clear which pages were generated by this framework vs. your hand-written notes. Page names with `/` create Logseq namespaces, which show up as a tree in the left sidebar. Themes (`javascript`, `outils`, etc.) are detected dynamically from your `pages/` directory — not fixed upfront.
...
| YouTube videos | `ingest-video` | Video transcript (yt-dlp) |
