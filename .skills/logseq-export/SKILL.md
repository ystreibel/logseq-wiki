...
You are exporting the wiki's wikilink graph to structured formats so it can be used in external tools (Gephi, custom scripts, browser visualization).

## Before You Start

**REQUIRED:** Invoke the `logseq-llm-wiki` skill first to understand Logseq syntax and file format rules.

1. Read `~/.logseq-wiki/config` to get `LOGSEQ_VAULT_PATH`
2. Confirm the vault has pages to export — if fewer than 5 pages exist in `wiki/**/*.md`, warn the user and stop
...
- Skip any page whose Logseq properties `tags::` contain a blocked tag when building the node list
...
Glob all `.md` files in `wiki/` subdirectory (e.g. `wiki/**/*.md`), excluding system directories. In filtered mode, also skip pages whose Logseq properties `tags::` contain `visibility/internal` or `visibility/pii`.

For each page, extract from Logseq properties (using `title::` syntax, NOT YAML frontmatter `title:`):
...
For each page, Grep the body for `\[\[wiki/` to extract all wikilinks to other pages in the wiki:
- Parse each `[[wiki/category/page]]` or `[[wiki/category/page|display]]` — use the target part only
...
- Dominant tag = the first tag in the page's Logseq `tags::` property
...
Create `wiki/_export/` directory if it doesn't exist. Write all three files:
...
```json
{
  ...
  "graph": {
    ...
    "vault": "<LOGSEQ_VAULT_PATH>",
    "total_nodes": N,
    "total_edges": M
  },
  "nodes": [
    {
      "id": "wiki/concept/transformers",
  ...
  "links": [
    {
      "source": "wiki/concept/transformers",
      "target": "wiki/entity/vaswani",
      "relation": "wikilink",
...
<graphml xmlns="http://graphml.graphdrawing.org/graphml">
  ...
  <graph id="wiki" edgedefault="undirected">
    <node id="wiki/concept/transformers">
    ...
    <edge source="wiki/concept/transformers" target="wiki/entity/vaswani">
...
```js
{id: "wiki/concept/transformers", label: "Transformer Architecture", color: {background: "#4E79A7"}, size: <degree * 3 + 8>, title: "concept | #ml #architecture", community: 0}
```
...
```js
{from: "wiki/concept/transformers", to: "wiki/entity/vaswani", dashes: false, width: 1, color: {color: "#666", opacity: 0.6}}
```
...
<head>
<meta charset="utf-8">
<title>Logseq Wiki Knowledge Graph</title>
...
<div id="graph"></div>
<div id="sidebar">
  <h3>Logseq Wiki Knowledge Graph</h3>
...
```
Export du graphe Logseq terminé → wiki/_export/ 📊
  graph.json    — N noeuds, M aretes (format NetworkX node_link)
  graph.graphml — N noeuds, M aretes (Gephi / yEd / Cytoscape)
  graph.html    — visualisation interactive en navigateur (ouvrir dans n'importe quel navigateur)
```

In filtered mode, append a line showing what was excluded:
```
  (filtre applique : X sur Y pages exclues — visibility/internal, visibility/pii)
```
...
- **The `wiki/_export/` directory should be gitignored** if the vault is version-controlled — these are derived artifacts
...
- **Logseq properties** are extracted using `property::` syntax, not YAML frontmatter — use `title::`, `tags::`, `summary::` from page content
