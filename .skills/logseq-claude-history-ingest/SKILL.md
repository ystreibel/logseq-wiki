...
description: >
  Ingest Claude Code conversation history into the Logseq wiki. Use this skill when the user wants to mine
...
# Claude History Ingest — Conversation Mining for Logseq

**REQUIRED:** Invoke `logseq-llm-wiki` skill first for Logseq syntax and file format rules.

You are extracting knowledge from the user's past Claude Code conversations and distilling it into the Logseq wiki. Conversations are rich but messy — your job is to find the signal and compile it.

This skill can be invoked directly or via the `logseq-history-ingest` router (`/logseq-history-ingest claude`).

## Before You Start

1. Read `~/.logseq-wiki/config` to get `LOGSEQ_VAULT_PATH` and `CLAUDE_HISTORY_PATH` (defaults to `~/.claude`)
2. Read `wiki/_manifest.json` at the vault root to check what's already been ingested
3. Read `wiki/_master-index.md` at the vault root to know what the wiki already contains

## Ingest Modes

### Append Mode (default)

Check `wiki/_manifest.json` for each source file (conversation JSONL, memory file). Only process:
...
Process everything regardless of manifest. Use after a `logseq-rebuild` or if the user explicitly asks.
...
Scan `CLAUDE_HISTORY_PATH` and compare against `wiki/_manifest.json`:
...
Write Logseq metadata using `::` properties at the top of each page:
...
category:: concept
source_projects:: [[myproject]], [[other-app]]
```

**Write a `summary::` property** on every new/updated page — 1–2 sentences, ≤200 chars, answering "what is this page about?" for a reader who hasn't opened it. This feeds cheap retrieval in `logseq-query`.

## Step 6: Mark Provenance

Mark provenance per the convention in `logseq-llm-wiki` (Provenance Markers section):
...
- Use `^[extracted]` for direct quotes or explicit statements from memory files.
- Write a `provenance::` property on every new/updated page summarizing the rough mix (example: "2 memory files (extracted), 4 conversations (inferred)").

## Step 7: Update Manifest, Journal, and Special Files
...
Update `wiki/_master-index.md` and `wiki/_log.md` per the standard process:
