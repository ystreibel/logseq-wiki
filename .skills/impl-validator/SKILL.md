...
4. **Convention check** — does it follow the project's established patterns?
   ...
   - Wiki pages: has all required Logseq property fields (`title::`, `category::`, `tags::`, `sources::`, `summary::`, `created::`, `updated::`) — sans YAML frontmatter (`---`)
...
## Logseq-Specific Checks

When validating wiki pages written for this project:

- Properties use `key:: value` syntax (never YAML `---` frontmatter)
- `_master-index.md` is the wiki root index (not `index.md`)
- Operation logs are written to `wiki/_log.md` (not `log.md`)
- Page links use `[[wiki/thème/page]]` namespace format
- All wiki content is written in French
