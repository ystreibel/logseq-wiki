...
1. **Resolve config** — read `~/.logseq-wiki/config` first; fall back to `.env` in the repo root. This gives `LOGSEQ_VAULT_PATH`, `WIKI_TOKEN_WARN_THRESHOLD` (default: `100000`), and `WIKI_STAGED_WRITES`.
2. Read `wiki/_manifest.json` at `$LOGSEQ_VAULT_PATH/wiki/_manifest.json` — this is the ingest tracking ledger.
3. Read `wiki/_log.md` for the activity journal.

## The Manifest

The manifest lives at `$LOGSEQ_VAULT_PATH/wiki/_manifest.json`. It tracks every source file that has been ingested. If it doesn't exist, this is a fresh vault with nothing ingested.
...
Build an inventory of everything available to ingest right now. Sources in logseq-wiki are the vault's own pages and journals, plus Claude history.

### Vault Pages and Journals (read-only sources)
```
Glob: $LOGSEQ_VAULT_PATH/pages/**/*.md   → user's pages
Glob: $LOGSEQ_VAULT_PATH/journals/*.md   → daily notes
...
**Visibility tally (before rendering the report):** Grep the `tags::` property across all wiki `.md` pages in `wiki/**/*.md` for `visibility/internal` and `visibility/pii` tag values. Count:
...
**Staged writes pending (only when `WIKI_STAGED_WRITES=true`):** Glob `$LOGSEQ_VAULT_PATH/wiki/_staging/**/*.md` and `**/*.patch.md`. Count new pages and patches separately. Report the oldest file's age.
...
| Source | Type | Size |
|---|---|---|
| $LOGSEQ_VAULT_PATH/pages/new-page.md | vault_page | 12 KB |
...
|---|---|---|---|
| $LOGSEQ_VAULT_PATH/pages/architecture.md | 2026-04-01 | 2026-04-05 | 4 days newer |
| ... | | | |
...
_Token estimate uses 4 chars/token heuristic — no actual tokenizer._
...
1. **Per-tier page sizes** — Glob all `.md` pages under `wiki/` (excluding `_master-index.md`, `_log.md`, `_insights.md`). Read the `tier::` Logseq property of each (cheap grep). Group pages by tier value (`core`, `supporting`, `peripheral`; unset → `supporting`).
...
Replace any single-line Recommendation with a ranked **What to Do Next** section. Gather these signals before rendering:

### 4a: Gather signals

0. **Staged writes pending** (only when `WIKI_STAGED_WRITES=true`) — Glob `$LOGSEQ_VAULT_PATH/wiki/_staging/**/*.md` and `**/*.patch.md`. Count new pages and patches separately. Report the oldest file's age (mtime). This is always listed first if any staged files exist — it has the highest intent signal (the LLM already did the work; the human just needs to review).

1. **`_raw/` files** — list every file in `$LOGSEQ_VAULT_PATH/wiki/_raw/` that isn't a `.gitkeep`. Count and name them.

2. **Stale core pages** — scan all wiki `.md` files. A page is "stale" when its `updated::` Logseq property is ≥90 days before today's date AND it has ≥5 incoming wikilinks (i.e., it's "core" — other pages depend on it). List them by name + last-updated date.

3. **Orphan pages** — pages with zero incoming wikilinks. To compute: glob all wiki `.md` pages, extract every `[[wikilink]]`, count references to each page, collect pages with `incoming == 0`. Show up to 5 names; report total count.

4. **Synthesis opportunities** — check `wiki/_log.md` for any recent `/wiki-synthesize` run summary. If the last synthesis run reported N opportunities, surface that count. If no synthesis has been run recently (not in `_log.md` within last 14 days), flag it as "synthesis scan overdue".

5. **Source delta** — from Step 2: count of new + modified sources ready to ingest.

6. **Lint issues** — check `wiki/_log.md` for a recent `/wiki-lint` run (within last 30 days). If a recent run recorded broken links or missing frontmatter, surface the count. If no lint run appears in the log, flag "lint not run recently".
...
|---|---|---|
| 0 | Staged writes pending | Any `.md`/`.patch.md` in `wiki/_staging/` (only when `WIKI_STAGED_WRITES=true`) |
| 1 | `_raw/` files waiting | Any files present in `wiki/_raw/` |
...
0. 📋  6 staged pages waiting for review (oldest: 3 days ago)
   → 4 new pages + 2 patches in wiki/_staging/
   run: /wiki-stage-commit

1. 📥  Ingest 3 files waiting in wiki/_raw/
   → architecture-notes.md, meeting-2026-05-10.md, paper-draft.pdf
   run: /wiki-ingest

2. 🔄  Refresh 2 stale core pages (not updated in 90+ days)
   → [[wiki/concepts/system-architecture]] (last updated 2026-02-10), [[wiki/concepts/api-design]] (2026-01-15)
   run: open these pages and re-run /wiki-update

3. 🔗  Link 7 orphan pages  →  run: /cross-linker
   Disconnected: [[wiki/concepts/redis-caching]], [[wiki/entities/jwt-tokens]], +5 more

4. 🧩  2 synthesis opportunities identified  →  run: /wiki-synthesize
   [[wiki/concepts/redis-caching]] × [[wiki/concepts/session-management]] (co-occur in 8 pages)
...
**First, build the wikilink graph.** Glob all `.md` pages under `wiki/`, extract every `[[wikilink]]`, and build:
- `incoming[page]` = count of other pages that link to this page
- `outgoing[page]` = count of pages this page links out to
- `tags[page]` = set of tags from `tags::` Logseq property
- `category[page]` = namespace prefix (wiki/concepts/, wiki/entities/, wiki/skills/, etc.)
...
7. **Graph delta since last run.** Compare the current link graph to the snapshot stored in the previous `wiki/_insights.md`:
   - Read the `<!-- GRAPH_SNAPSHOT: ... -->` line at the bottom of the previous `wiki/_insights.md` (if it exists) — it contains a compact JSON edge list
...
8. **Tier assignment suggestions.** After computing hubs and bridges, recommend `tier::` changes. Never write `tier::` to pages — only surface suggestions so the human can decide.
   ...
   - Show up to 10 suggestions (promotions first, then demotions), formatted as:
     ```
     Tier Suggestions:
     ↑ core    [[wiki/concepts/attention-mechanism]] — 14 incoming links, currently tier=supporting
     ↑ core    [[wiki/entities/andrej-karpathy]]     — bridge (3 cluster pairs), currently unset
     ↓ peripheral [[wiki/concepts/old-concept]]       — 0 incoming, 120 days stale
     ```
...
Write the result to `wiki/_insights.md` at `$LOGSEQ_VAULT_PATH/wiki/_insights.md`. Overwrite freely — it's regenerable. At the very end, embed a compact graph snapshot as an HTML comment so the next run can diff against it.
...
## Tier Suggestions
↑ core    [[wiki/concepts/attention-mechanism]] — 14 incoming links, currently tier=supporting
↑ core    [[wiki/entities/andrej-karpathy]]     — top bridge (4 cluster pairs), currently unset
↓ peripheral [[wiki/concepts/old-concept]]       — 0 incoming, 132 days stale
...
After writing the file, append to `wiki/_log.md`:
```
...
- Vaults with fewer than 20 wiki pages — not enough graph structure. Tell the user and skip.
...
- This skill only reads and reports — it doesn't modify anything (except writing `wiki/_insights.md` in insights mode, which is regenerable)
- The actual ingest work is done by the ingest skills (`wiki-ingest`, `wiki-update`, `claude-history-ingest`, `data-ingest`)
- Those skills are responsible for updating the manifest after they finish
- Never modify `pages/`, `journals/`, `assets/`, or `logseq/` — these are read-only vault sources
