...
You are helping the user browse and compare their Logseq wiki knowledge filtered by which AI tool originally produced it. The wiki tracks source provenance in `wiki/_manifest.json` and page `sources::` properties — this skill surfaces that metadata as a navigable view.

## Before You Start

1. **Resolve config** — read `~/.logseq-wiki/config` (first); if absent, read `.env` in the logseq-wiki repo (fallback); if neither exists, prompt setup. This gives `LOGSEQ_VAULT_PATH`.
2. Read `$LOGSEQ_VAULT_PATH/wiki/_manifest.json` — this is the source-of-truth for what tool produced what.
3. Read `$LOGSEQ_VAULT_PATH/wiki/_master-index.md` for page titles and one-line descriptions.
...
Read `wiki/_manifest.json`. For each source entry, extract:
...
|------|----------|------|--------------|
| [[wiki/thème/page]] | concept | tag1, tag2 | 2026-04-10 |
...
```

Read Logseq properties for the listed pages (grep for `^(title|category|tags|updated)::`) — do not read full page bodies unless the user asks.
...
<list with one-line descriptions from _master-index.md>
...
|------|--------|-------|--------|---------|----|
| [[wiki/concepts/react-patterns]] | ✓ | ✓ | — | ✓ | — |
| [[wiki/concepts/rust-ownership]] | — | ✓ | — | — | ✓ |
```
...
impl-validator check:
  ...
  checks:
    - Did you correctly parse source_type from wiki/_manifest.json?
...
- Use `[[wiki/thème/page]]` namespace links for all page references.
- In diff mode, call out the most *surprising* asymmetry explicitly — that's the insight the user came for.
- If `wiki/_manifest.json` is empty or missing, say so clearly and suggest running `/wiki-history-ingest` first.
